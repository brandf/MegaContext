...
> - `--mc_num_random_variants` and `--mc_train_wc_length` set how many stochastic WC compressions we sample per sequence and how aggressively each one shrinks toward the training working-context length (we linearly anneal from 80 % of `max_seq_len` down to `mc_train_wc_length`, which defaults to 20 %, e.g. 1,536 → 409 when `max_seq_len=2048`). When `--mc_auto_batch` is enabled we heuristically scale `device_batch_size` using the variant count and target WC length so MC runs stay near the baseline VRAM envelope without manual tuning.
...
> - `--mc_auto_batch 1` (default) automatically shrinks `device_batch_size` based on that heuristic (with a small safety margin) so flipping `--mc` on doesn’t unexpectedly OOM; set it to `0` if you want to control batch math yourself.
