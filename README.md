...
> - `--mc_num_random_variants`, `--mc_train_wc_length`, `--mc_max_counterfactuals` tune how many random WC compressions we sample per sequence and how aggressively each one shrinks toward the training working-context length (we linearly anneal from 80 % of `max_seq_len` down to `mc_train_wc_length`, which defaults to 20 %, e.g. 1,536 → 409 when `max_seq_len=2048`).
