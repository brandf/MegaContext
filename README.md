...
> - `--mc_num_random_variants`, `--mc_train_wc_length`, `--mc_max_counterfactuals` tune how many random WC compressions we sample per sequence and how aggressively each one shrinks toward the training working-context length (defaults to 75 % of `max_seq_len`, e.g. 1,536 when `max_seq_len=2048`).
