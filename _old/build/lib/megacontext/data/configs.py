...
class DatasetConfig(BaseModel):
    ...
    dataset_name: str = Field(description="Identifier used for output metadata directories.")
    tokenizer: str = Field(description="Hugging Face tokenizer name or local path.")
    block_size: int = Field(default=32, description="Number of L0 tokens per block.")
    ...
    @classmethod
    def ensure_named_split(cls, value: dict[str, SplitConfig]) -> dict[str, SplitConfig]:
        if not value:
