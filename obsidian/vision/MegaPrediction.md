...
- Predicted gists and draft LOD0 tokens occupy normal tree nodes marked as speculative; the cursor keeps runtime code from confusing unverified futures with committed history.
...
- Run coarse reasoning entirely in gist space, pruning LOD0 details to keep planning cheap.
...
- Seed a draft with an LOD2 gist sequence, pass it through a [[DeGistNet]]-style decoder to reach LOD1, and apply diffusion-style refinement to squeeze entropy before landing on LOD0 tokens.
...
- Combine latent planning and hierarchical output: start in LOD2+, let [[LensNet]] score speculative spans, and only refine the segments that matter.
...
- Reuse ΔNLL-style losses by comparing refined LOD0 outputs against ground-truth continuations, avoiding the need to score intermediate gists directly.
