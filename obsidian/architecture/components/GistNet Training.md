...
The core objective ensures that replacing a 32-token span with its gist embedding causes minimal change in the base [[Glossary#Base Model|LLM]]'s output distribution. This is measured via [[Glossary#ΔNLL (Delta Negative Log-Likelihood)|ΔNLL]]@H—the change in negative log-likelihood over a prediction horizon H.
...
Equivalently, minimize the [[Glossary#ΔNLL (Delta Negative Log-Likelihood)|ΔNLL]] between full and gist-replaced contexts:
...
**Role:** Compute prediction distributions for [[Glossary#ΔNLL (Delta Negative Log-Likelihood)|ΔNLL]]@H evaluation

**Requirements:**
- Same architecture as the target base [[Glossary#Base Model|LLM]] (e.g., `HuggingFaceTB/SmolLM3-3B`)
...
**Metrics to monitor:**
- [[Glossary#ΔNLL (Delta Negative Log-Likelihood)|ΔNLL]]@H < 0.5 nats (gist causes minimal prediction degradation)
...
**Metrics to monitor:**
- [[Glossary#ΔNLL (Delta Negative Log-Likelihood)|ΔNLL]]@H < 1.0 nats (allow slightly higher error for structured data)
...
**Metrics to monitor:**
- [[Glossary#ΔNLL (Delta Negative Log-Likelihood)|ΔNLL]]@H < 0.8 nats (weighted average across text types)
...
Prevents gradient explosion when [[Glossary#ΔNLL (Delta Negative Log-Likelihood)|ΔNLL]]@H spikes (e.g., teacher produces low-probability predictions).
...
1. **Teacher-student distillation [2]:** [[GistNet]] learns by minimizing [[Glossary#ΔNLL (Delta Negative Log-Likelihood)|ΔNLL]]@H, not by reconstructing tokens.
...
- [[Glossary#ΔNLL (Delta Negative Log-Likelihood)|ΔNLL]] – Core evaluation metric: delta in negative log-likelihood for substitutability
- [[Glossary#Substitutability|substitutability]] – Foundational design principle that drives training objectives
...
- [[MegaContext End-to-End Training]] – How GistNet training alternates with LensNet and tree operations
...
- [[Context Focus]] – Initial setup including GistNet training pipeline
