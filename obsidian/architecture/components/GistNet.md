...
GistNet is trained to ensure **substitutability** [1]: replacing a span with its gist should minimally change the base LLM's predictions. The primary objective minimizes [[Glossary#ΔNLL (Delta Negative Log-Likelihood)|ΔNLL]]@H between original and gist-replaced contexts. The training process uses knowledge distillation [3] techniques to align gist representations with the base LLM's internal representations.
...
- [[Glossary#ΔNLL (Delta Negative Log-Likelihood)|ΔNLL]] — Primary training metric for substitutability
- [[Glossary#Substitutability|substitutability]] — Core principle guiding GistNet design
