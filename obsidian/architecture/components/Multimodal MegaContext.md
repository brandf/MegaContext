...
---
> _Diagram TODO:_ add multimodal MegaContext schematic once assets are available.
...
- **Global index:** Every image ingest event receives a base index, keeping chronological ordering consistent with text and allowing ALiBi biases to operate across modalities. See [[Positional Encoding#Track B — Retrofit path for upgrading frozen bases (backlog)|Positional Encoding Track B]] for the compatibility hooks we still maintain.
...
- **Gaussian extension:** When [[Positional Encoding#Gaussian RoPE stack|Gaussian RoPE]] drives Track A, image gists set σ terms based on tile width/height, naturally blurring high frequencies in both axes.
