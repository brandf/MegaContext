...
| Telemetry | Prior to this review the only signals were `mc/lens_loss`, `mc/delta_mean/p95`. | New `--mc_log_lens_debug` instrumentation now surfaces score stats + correlations, revealing the polarity bug (scores anti-correlate with ΔNLL). |
