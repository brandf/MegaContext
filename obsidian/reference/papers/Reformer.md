...
- **Memory-efficient staging context**: [[Glossary#Staging Context|Staging Context]] with 100k entries would benefit enormously from reversible attention—current dense attention over staging would be prohibitively expensive.

## What We Can Use
- **Implement LSH attention for LensNet-G**: When [[LensNet]] operates over the large [[Glossary#Staging Context|Staging Context]], use LSH to bucket entries by semantic similarity. Only compute full attention scores within top-k buckets, reducing compute by 10-50×.
...
- **Poor performance on short contexts**: LSH overhead (hashing, sorting, bucketing) dominates when n is small. Not useful for [[Working Context]] (W_max=8k), only for large [[Glossary#Staging Context|Staging Context]] or disk-backed [[MegaContext Tree]] scans.
...
- [[Glossary#Staging Context|Staging Context]]
- [[MegaContext Tree]]
- [[Storage Format]]
- [[Performance Sketch]]
- [[Focus Allocator]]
