...
Procedure (on-policy):
1. Build/refresh [[MegaContext Tree]] with current `Gist`.
...
1. **Setup:** Load a small TypeScript project summary into [[MegaContext Tree]] memory (≈4k tokens) and seed the [[Working Context]] with the latest user/system gists.
...
Document a similar narrative under `docs/walkthroughs/` once the [[POC Plan]] code path is live so future contributors can replay it end to end.
